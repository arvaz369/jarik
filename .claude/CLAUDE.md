...
- **A) На этот компьютер** — прочитай `INSTALL.md` и проведи интервью из 10 вопросов.
- **B) На сервер (VPS)** — сначала прочитай `INSTALL.md` и проведи интервью (10 вопросов, заполнение шаблонов). После завершения прочитай `server/INSTALL-SERVER.md` и перенеси всё на сервер пользователя. Понадобятся: IP сервера и пароль root.
