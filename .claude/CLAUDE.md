...
**Читай:** `knowledge/quality-control.md`

Каждый артефакт (код, документ, план, дизайн) проходит проверку по 5 критериям **перед выдачей**:

1. ✅ Требования выполнены?
2. ✅ Функциональность работает?
3. ✅ Качество кода/текста?
4. ✅ Безопасность?
5. ✅ Готово к использованию?

**Результат не показывается пока все критерии не пройдены.**

Это гарантирует: что получишь — работает и готово к клиенту. Добавляет ~15-25% времени, но экономит часы на исправления потом.

Использование:
- Для кода: `/qa` (тестирование) + `/review` (качество)
- Для документов: проверка ссылок, данные, язык
- Для планов: логика, конкретность, порядок шагов
